...
import { Check, Star, TrendingUp, Clock, DollarSign, Shield, ArrowLeft } from 'lucide-react';
...
export default function InvestmentPlansPage() {
  ...
  const confirmInvestment = () => {
    ...
    if (amount < selectedPlan.minAmount || amount > selectedPlan.maxAmount) {
      toast.error(`Investment amount must be between $${selectedPlan.minAmount} and $${selectedPlan.maxAmount}`);
      return;
    }
      ...
      <main className="pt-24 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                  ...
                  <div className="text-center mb-6">
                    ...
                    <div className="flex items-baseline justify-center gap-1">
                      <span className="text-4xl font-bold text-crypto-yellow">
                        {plan.dailyReturn}%
                      </span>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            <div className="p-6 rounded-2xl bg-crypto-card border border-crypto-border">
              ...
              <p className="text-gray-400 text-sm">
                Your investments are protected by our advanced security systems and insurance coverage.
              </p>
            </div>
            <div className="p-6 rounded-2xl bg-crypto-card border border-crypto-border">
              <div className="w-12 h-12 rounded-xl bg-crypto-yellow/10 flex items-center justify-center mb-4">
                <TrendingUp className="w-6 h-6 text-crypto-yellow" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Daily Returns</h3>
              <p className="text-gray-400 text-sm">
                Earn consistent daily returns that are automatically credited to your account.
              </p>
            </div>
            <div className="p-6 rounded-2xl bg-crypto-card border border-crypto-border">
              <div className="w-12 h-12 rounded-xl bg-crypto-yellow/10 flex items-center justify-center mb-4">
                <Clock className="w-6 h-6 text-crypto-yellow" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Flexible Terms</h3>
              <p className="text-gray-400 text-sm">
                Choose from various investment durations that suit your financial goals.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6 py-4">
            <div className="p-4 rounded-xl bg-crypto-dark border border-crypto-border">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-500 font-bold">
                  {selectedPlan ? selectedPlan.dailyReturn * selectedPlan.duration : 0}%
                </span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-crypto-yellow/10 border border-crypto-yellow/30">
              <div className="flex justify-between">
                ...
                <span className="text-crypto-yellow font-bold">
                  ${selectedPlan ? (parseFloat(investmentAmount || '0') * selectedPlan.dailyReturn / 100).toFixed(2) : '0.00'}
                </span>
              </div>
              <div className="flex justify-between mt-2">
                <span className="text-gray-300">Total Earnings</span>
                <span className="text-green-500 font-bold">
                  ${selectedPlan ? (parseFloat(investmentAmount || '0') * selectedPlan.dailyReturn * selectedPlan.duration / 100).toFixed(2) : '0.00'}
                </span>
              </div>
            </div>
